...
// long-running bitmemedMain (which is the real meat of bitmemed), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start bitmemedMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once bitmemed is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("bitmemedMain-windows", func() {
...
// logServiceStart logs information about bitmemed when the main server has
